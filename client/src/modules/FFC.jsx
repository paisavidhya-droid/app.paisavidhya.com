  ...
  return (
    <div className="pv-row" style={{ alignItems: "center", gap: 8 }}>
      <label style={{ width: "60%", color: "var(--pv-dim)" }}>
        ...
        {hint ? (
          <span
            style={{ marginLeft: 6, fontSize: 12, color: "var(--pv-muted)" }}
          >
            ({hint})
          </span>
        ) : null}
...
export default function FFC() {
  ...
  const savingsMonthly =
    fmt(obligations.sip) + fmt(obligations.rd) + fmt(goals.kidsFund);
  ...
  const members =
    fmt(profile.adults) + fmt(profile.children) + fmt(profile.seniors);
  const minHealthCover =
    members <= 3 ? 500000 : members <= 5 ? 1000000 : 1500000; // suggested floater
  const hasAdequateHealth =
    fmt(protection.healthInsurance) + fmt(protection.parentsHealth) >=
    minHealthCover / 12; // monthly equivalent approximation
  ...
  const coreMonthly =
    ...
    sumObj(transport) +
    sumObj(protection);
  const emergencyMonths = coreMonthly
    ? fmt(cushions.emergencyFund) / coreMonthly
    : 0;
  ...
  const insights = useMemo(() => {
    ...
    if (savingsRate < 20)
      list.push({
        ...
        detail: "Aim for 20–30% savings. Tighten lifestyle or increase income.",
      });
    if (dti > 30)
      list.push({
        type: "danger",
        title: "High Debt-to-Income",
        detail: "EMIs exceed 30% of income. Consider prepaying or refinancing.",
      });
    if (surplus < 0)
      list.push({
        type: "danger",
        ...
        detail:
          "Expenses exceed income. Trim variable spends and reassess goals.",
      });

    if (!hasAdequateHealth)
      list.push({
        type: "info",
        title: "Health Cover May Be Low",
        detail: `For a family of ${members}, target floater ≈ ₹${minHealthCover.toLocaleString("en-IN")}.`,
      });

    if (emergencyMonths < 3)
      list.push({
        type: "warning",
        title: "Emergency Fund Thin",
        detail: `Maintain 3–6 months of core expenses; you have ~${emergencyMonths.toFixed(1)} months.`,
      });

    if (fmt(protection.lifeInsurance) * 12 * 1000 < lifeCoverNeeded)
      list.push({
        type: "info",
        title: "Life Cover Check",
        detail: `Indicative need ~10× annual income. Proxy needed ≈ ₹${(lifeCoverNeeded / 100000).toFixed(0)}L SA.`,
      });

    const lifestyleShare = totalExpenses
      ? sumObj(lifestyle) / totalExpenses
      : 0;
    if (lifestyleShare > 0.3)
      list.push({
        type: "info",
        title: "Lifestyle Spend High",
        detail: "Lifestyle >30% of expenses. Audit subscriptions and outings.",
      });
  ...
  return (
    <>
      <ModuleHeader
        title="Family Finance Checkup"
        subtitle="Capture household income & expenses to understand surplus and key ratios."
        actions={
          <>
      ...
      >
        ...
        <Card title="Family Profile">
          <div className="pv-row" style={{ flexWrap: "wrap", gap: 12 }}>
            ...
            <Input
              ...
              onChange={(e) =>
                setProfile({ ...profile, adults: Number(e.target.value || 0) })
              }
            />
            <Input
              type="number"
            ...
            <Input
              type="number"
              label="Seniors"
              value={profile.seniors}
              onChange={(e) =>
                setProfile({ ...profile, seniors: Number(e.target.value || 0) })
              }
            />
          </div>
        </Card>
          ...
          <div className="pv-col" style={{ gap: 8 }}>
            ...
            {totalIncome > 0 && (
              <div className="pv-row" style={{ justifyContent: "flex-end" }}>
                <Alert
                  title={`Income — ₹${totalIncome.toLocaleString("en-IN")}`}
                />
              </div>
            )}
          </div>
        ...
        <Accordion
          items={[
            {
              title: renderAccTitle("Housing", sumObj(housing)),
              content: (
                <CategoryGroup
                  title="Housing"
                  fields={[
            ...
            {
              title: renderAccTitle("Family Needs", sumObj(familyNeeds)),
              content: (
                <CategoryGroup
                  title="Family Needs"
                  fields={[
                  ...
                  data={familyNeeds}
                  setData={setFamilyNeeds}
                />
              ),
            },

            {
              title: renderAccTitle("Transport", sumObj(transport)),
              content: (
                <CategoryGroup
                  title="Transport"
                  fields={[
              ...
              content: (
                <CategoryGroup
                  ...
                  fields={[
                    ...
                    {
                      key: "healthInsurance",
                      label: "Family Health Premium (monthly)",
                    },
                    {
                      key: "parentsHealth",
                      label: "Parents Health Premium (monthly)",
                    },
                  ]}
                  data={protection}
                  setData={setProtection}
                />
              ),
            },

            {
              title: renderAccTitle("Lifestyle", sumObj(lifestyle)),
              content: (
                <CategoryGroup
                  title="Lifestyle"
                  fields={[
            ...
            {
              title: renderAccTitle("Cushions", sumObj(cushions)),
              content: (
                <CategoryGroup
                  title="Cushions"
                  fields={[
        ...
        <Card title="Family Financial Summary">
          ...
          >
            ...
            <div className="pv-col" style={{ gap: 6, minWidth: 240 }}>
              ...
              <Badge>
                Debt-to-Income {totalIncome > 0 ? `${dti.toFixed(1)}%` : "-"}
              </Badge>
