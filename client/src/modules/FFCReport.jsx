...
export default function FFCReport() {
  ...
  if (!state) {
    return (
      <div className="pv-col pv-container" style={{ gap: 16, padding: 16 }}>
        <ModuleHeader title="FFC Report" subtitle="Preview & Export" />
  ...
  const savingsRate =
    metrics?.savingsRate ??
    (totalIncome
      ? ((Number(obligations?.sip || 0) +
          Number(obligations?.rd || 0) +
          Number(goals?.kidsFund || 0)) /
          totalIncome) *
        100
      : 0);
  const dti =
    metrics?.dti ??
    (totalIncome ? (Number(obligations?.loanEmi || 0) / totalIncome) * 100 : 0);
  ...
  const emergencyMonths =
    ...
    (coreMonthly
      ? Number(state?.cushions?.emergencyFund || 0) / coreMonthly
      : 0);
  ...
  const insights = useMemo(() => {
    ...
    if (savingsRate < 20)
      list.push({
        ...
        detail: "Target 20–30% savings.",
      });
    if (dti > 30)
      list.push({
        type: "danger",
        title: "High Debt-to-Income",
        detail: "EMIs exceed 30% of income.",
      });
    if (surplus < 0)
      list.push({
        type: "danger",
    ...
    const members =
      Number(profile?.adults || 0) +
      Number(profile?.children || 0) +
      Number(profile?.seniors || 0);
    const minHealthCover =
      members <= 3 ? 500000 : members <= 5 ? 1000000 : 1500000;
    ...
    if (lifestyleShare > 0.3)
      list.push({
        ...
        detail: ">30% of expenses are lifestyle.",
      });
    list.push({
      type: "info",
      title: "Suggested Family Floater",
      detail: `≈ ₹${inr.format(minHealthCover)} (indicative).`,
    });
    return list;
  }, [
    totalIncome,
  ...
  return (
    <div className="pv-col pv-container" style={{ gap: 16, padding: 16 }}>
      <ModuleHeader
        ...
        actions={
          <>
            <Button variant="ghost" onClick={() => navigate("/ffc")}>
              Edit
            </Button>
      ...
      <div ref={printRef} className="pv-col" style={{ gap: 16 }}>
        ...
        <Card className="print-block">
          ...
          >
            <div>
              <h2 style={{ margin: 0 }}>
                Paisavidhya — Family Financial Checkup
              </h2>
        ...
        <Card title="Family Profile" className="print-block">
          <div className="grid-3">
            ...
            <div className="kv">
              ...
              <div>
                {profile?.adults || 0} Adults, {profile?.children || 0}{" "}
                Children, {profile?.seniors || 0} Seniors
              </div>
            </div>
          </div>
        </Card>
            ...
            <tbody>
              {Object.entries(income || {}).map(([k, v]) => (
                <tr key={k}>
                  ...
                  <td>{inr.format(Number(v) || 0)}</td>
                </tr>
              ))}
              <tr>
                <td style={{ fontWeight: 600 }}>Total</td>
                <td style={{ fontWeight: 600 }}>{inr.format(totalIncome)}</td>
              </tr>
            </tbody>
          </table>
        </Card>
        ...
        <Card title="Expenses (Monthly)" className="print-block">
          <table className="table">
            ...
            <tbody>
              <tr>
                <td>Housing</td>
                <td>{inr.format(sumObj(housing))}</td>
              </tr>
              <tr>
                <td>Family Needs</td>
                <td>{inr.format(sumObj(familyNeeds))}</td>
              </tr>
              <tr>
                <td>Transport</td>
                <td>{inr.format(sumObj(transport))}</td>
              </tr>
              <tr>
                <td>Protection</td>
                <td>{inr.format(sumObj(protection))}</td>
              </tr>
              <tr>
                <td>Lifestyle</td>
                <td>{inr.format(sumObj(lifestyle))}</td>
              </tr>
              <tr>
                <td>Obligations</td>
                <td>{inr.format(sumObj(obligations))}</td>
              </tr>
              <tr>
                <td>Goals</td>
                <td>{inr.format(sumObj(goals))}</td>
              </tr>
              <tr>
                <td style={{ fontWeight: 600 }}>Total</td>
                <td style={{ fontWeight: 600 }}>{inr.format(totalExpenses)}</td>
              </tr>
            </tbody>
          </table>
        </Card>
        ...
        {insights?.length ? (
          <Card title="Insights & Recommendations" className="print-block">
            ...
            <ul style={{ margin: 0, paddingLeft: 16 }}>
              <li>
                Target savings 20–30% via SIP/PPF/NPS; automate on salary day.
              </li>
              <li>
                Keep 3–6 months of core expenses in an emergency fund
                (liquid/overnight).
              </li>
              <li>
                Ensure adequate term life (≈10× annual income) and family
                floater health cover.
              </li>
              <li>
                Cap lifestyle spends at ≤30% of expenses; audit subscriptions
                quarterly.
              </li>
              <li>
                Prepay high-interest debt before new goals; keep DTI ≤30%.
              </li>
            </ul>
          </Card>
        ) : null}
        ...
        <Card className="print-block">
          ...
          >
            <div>
              ...
              <div className="muted">
                Add personalized observations here during session.
              </div>
            </div>
